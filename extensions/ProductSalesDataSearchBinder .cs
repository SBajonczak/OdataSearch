...
namespace odataQuery.Extensions
{
    public class ProductSalesDataSearchBinder  : QueryBinder, ISearchBinder
    {
        ...
        public Expression BindSearch(SearchClause searchClause, QueryBinderContext context)
        {
            ...
            if (node != null)
            {
                ...
                if (context.ElementClrType == typeof(Product))
                {
                    Expression<Func<Product, bool>> exp = p => p.Name.ToLowerInvariant().Contains(node.Text.ToLowerInvariant());
                    return exp;
                }
